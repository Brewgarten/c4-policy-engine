  ...
  stages {
    ...
    stage("retrieve dependencies") {
      steps {
        parallel (
          ...
          "c4-systemmanager": {
            dir("lib/c4/system-manager") {
              git credentialsId: "github", url: "https://github.ibm.com/c4/system-manager"
            }
            sh 'cd lib/c4/system-manager && git checkout tags/$(git describe --abbrev=0 --tags)'
          },
          "c4-utils": {
            dir("lib/c4/utils") {
    ...
    stage("code analysis") {
      steps {
        ...
        sh "env/bin/pip install --disable-pip-version-check --no-cache-dir lib/c4/system-manager"
